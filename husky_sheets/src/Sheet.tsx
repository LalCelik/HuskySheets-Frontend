...
function Sheet() {
  ...
  const [empListUpdates, setUpdates] = useState<string[]>([]);
  ...
  const [url, setUrl] = useState('http://localhost:8080/api/v1/getUpdatesForSubscription');
  ...
  const handleCellBlur = (event: Event) => {
    const cell = event.target as HTMLTableCellElement;
      ...
      if (gridInstance) {
        ...
        try {
          if (newValue.includes("COPY"))  {
            ...
            var rowIdx = (str?.numeric || 0) + 1;
            const colName = generateColumnName(colIdx);
  ...
  function updateFormulaCell(columnName: string, rowIndex: number) {
  ...
  const data: (string | number)[][] = Array.from({ length: 100 }, (_, rowIndex) => {
    const row: (string | number)[] = Array.from({ length: 100 - 1 }, () => '')
    return [rowIndex, ...row];
  });

  const displayData: (string | number)[][] = Array.from({ length: 100 }, (_, rowIndex) => {
    const row: (string | number)[] = Array.from({ length: 100 - 1 }, () => '')
    return [rowIndex, ...row];
  });


  function reverseParse(listUpdates: string[]) {
    var finalData: (string | number)[][] = Array.from({ length: 100 }, (_, rowIndex) => {
      const row: (string | number)[] = Array.from({ length: 100 - 1 }, () => '')
      return [rowIndex, ...row];
    });
    const regex = /([A-Z]+)(\d+)/;
    ...
    for (let i = 0; i < data.length; i++) {
      for (let j = 0; j < data[i].length; j++) {
        ...
        if (cellValue[0] === "=") {
          if (cellValue.includes("DEBUG")) {
            origDebug = origDebug.concat(cellValue);
            data[i][j] = cellValue.replace(/\bDEBUG\b/g, '').trim();
          }
          var refList = [`${i}-${j}`];
          if (cellValue.includes("COPY"))  {
            const tokens = getCellsInFormula(cellValue.toUpperCase());
            const newTokens = getCellsInFormula(RefToNumberFormula(cellNamePattern, dels, data, cellValue, refList));
            const refCell = tokens[5];
            const copyVal = newTokens[3];
            var str = splitString(refCell);
            var colIdx = columnNameToIndex(str?.letter) - 1;
          ...
          try {
            displayData[i][j] = '' + FormulaParse(RefToNumberFormula(cellNamePattern, dels, data, cellValue, refList)).value;
            if (origDebug.includes("DEBUG")) {
  ...
  const getUpdates = () => {
    var pubData: (string | number)[][] = Array.from({ length: 100 }, (_, rowIndex) => {
      const row: (string | number)[] = Array.from({ length: 100 - 1 }, () => '')
      return [rowIndex, ...row];
    });
    const regex = /([A-Z]+)(\d+)/;
        ...
        for (let sheetIdx = 0; sheetIdx < data.value.length; sheetIdx++) {
          ...
          for (let updateIdx = localRecentUpdate; updateIdx < allUpdates.length - 1; updateIdx++) {
            ...
            if (match) {
              ...
              const colIdx = columnNameToIndex(cellCoords.letter);
              const rowIdx = cellCoords.number;
              const cellIndex = columnNameToIndex(String(colIdx));
              const columnName = generateColumnName(cellIndex);
              const gridInstance = gridContainerRef.current;
              if (gridInstance) {
    ...
    <div className="SheetPage">
      <header className="Sheet-header">
        ...
        <div className="header-title">
          ...
          <h2>{sheetName}</h2>
          <span className="owner-badge">by {dataPublisher}</span>
        </div>
        ...
        <div className="grid-container" ref={gridContainerRef}>
          <Grid
            ...
            server={{
              ...
              then: (data: { value: { payload: string }[] }) => {
